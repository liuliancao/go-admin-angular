...
export class PersonalizeService {
  ...
  initTheme() {
    if (window['devuiThemes']) {
      //TODO 组件库开源版本这个色值错误
      window['devuiThemes'].devuiDarkTheme.data['devui-placeholder'] =
        '#8A8A8A';
      ...
      // 字号设置
      const fontId =
        localStorage.getItem('font') || this.configs[1].items[0].id;
      const radiusId =
        localStorage.getItem('radius') || this.configs[2].items[0].id;
